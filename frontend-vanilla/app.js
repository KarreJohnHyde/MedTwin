...
function updateDashboard(msg) { 
  ...
  window.currentForecast = Array.isArray(msg.forecast) ? msg.forecast : [];
  ...
  const visionMetrics = msg.vision_analysis?.metrics; if (visionMetrics) $('vision-metrics').textContent = `Held-out: AUC ${Number(visionMetrics.roc_auc).toFixed(3)} · F1 ${Number(visionMetrics.f1).toFixed(3)} · ${visionMetrics.test_images} images`;
...
$('forecast-slider').addEventListener('input', e => {
  const step = +e.target.value;
  const forecast = window.currentForecast || [];
  const risk = forecast[step] ?? (lastRisk / 100);
  $('forecast-day').textContent = `Step ${step}`;
  $('forecast-caption').textContent = forecast.length ? `Observed-history forecast · projected risk ${Math.round(risk * 100)}%` : 'Awaiting observed risk history for forecast';
});
$('run-inference').onclick = () => {
  if ($('model-selector').value === 'vision-cnn') $('analyze-mri').click();
  else $('workspace-result').textContent = 'Supply real clinical inputs to run this model.';
};
$('analyze-mri').onclick = () => {
  const file = $('mri-file').files[0];
  const result = $('workspace-result');
  if (!file) { result.textContent = 'Choose a PNG or JPEG MRI image first.'; return; }
  result.textContent = 'Running trained brain-MRI CNN…';
  const reader = new FileReader();
  reader.onload = async () => {
    try {
      const response = await fetch('/api/v1/vision/analyze', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({patient_id:PATIENT_ID, image_base64:reader.result})});
      const data = await response.json();
      if (!response.ok) throw Error(data.detail || 'Analysis failed');
      const metrics = data.vision_result.metrics;
      $('vision-metrics').textContent = `Held-out: AUC ${Number(metrics.roc_auc).toFixed(3)} · F1 ${Number(metrics.f1).toFixed(3)} · ${metrics.test_images} images`;
      result.textContent = `${data.vision_result.finding} · confidence ${Math.round(data.vision_result.confidence * 100)}% · non-localized image classification.`;
      updateDashboard(data.fusion_update);
    } catch (error) { result.textContent = `MRI analysis unavailable: ${error.message}`; }
  };
