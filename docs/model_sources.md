...
| Artifact | Held-out evaluation | Data safeguards |
| --- | --- | --- |
| `artifacts/heart_xgboost.joblib` | ROC-AUC 0.8452, F1 0.7500, accuracy 0.7377 (61 rows) | 723 exact duplicate rows removed before a stratified split; 302 unique rows remained. |
| `artifacts/brain_tumor_cnn.pt` | ROC-AUC 0.8776, F1 0.8500, accuracy 0.8286 (35 images) | exact duplicate images grouped and reduced to one representative; 228 unique image-content groups split 159/34/35. |

The brain artifact is connected to `POST /api/v1/vision/analyze` and the 3-D
brain viewer. It reports a binary image-classification pattern, not a lesion
location. Its small, local dataset is appropriate only for research prototyping.
